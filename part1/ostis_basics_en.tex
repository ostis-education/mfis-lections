...
\title{Lecture 1 \\Basics of Semantic Technology for Next-Generation Intelligent Computer Systems Design \vspace{-2em}}
...
\centering
\Huge
\textbf{The concept of a next-generation of intelligent computer system}
\end{frame}
...
\footnotesize
\scnheader{cybernetic system}
\scnrelfrom{subdividing}{A feature of the naturalness or artificiality of cybernetic systems}
\begin{scnindent}
\begin{scneqtoset}
...
\scnitem{intelligent community of intelligent computer systems}
\begin{scnindent}
	\scnidtf{intelligent \textit{multi-agent system} whose agents are \textit{intelligent computer systems}}
	\scntext{note}{Not every \textit{community of intelligent computer systems} can be intelligent, since the level of intelligence of such a community is determined not only by the level of intelligence of its members, but also by the effectiveness (quality) \uline{of their interactions}.}
	\begin{scnrelfromset}{subdividing}
		\scnitem{intelligent community of \uline{individual} intelligent computer systems}
		\scnitem{hierarchical intelligent community of intelligent computer systems}
	\end{scnrelfromset}
\end{scnindent}
\end{scnrelfromset}
\end{SCn}
...
\scnidtf{a system that is capable of solving intelligent problems}

\scnheader{intelligent problem}
\scnidtf{a problem for which there is no known solution algorithm}

\scnheader{intelligent system}
\scnidtf{a system that is able learn}
\end{SCn}
\end{frame}
...
\begin{SCn}
\scnheader{intelligent system}
\scnidtf{a system that can easily \underline{learn to solve} new types of problems}
\begin{scnrelfromset}{important to distinguish}
	\scnitem{the ability to learn better \underline{quality solutions to} problems of \underline{one limited} class (as neural network models do)}
	\scnitem{the ability to learn to solve problems of \underline{different} classes (with or without limitations)}
\end{scnrelfromset}
\end{SCn}
\end{frame}
...
\begin{scnrelfromlist}{requirements}
	\scnitem{high level of \textit{interoperability}}
	\scnitem{high level of \textit{learning ability}}
	\scnitem{high level of \textit{hybridity}}
	\scnitem{high level of ability to solve \textit{intelligent problems} (that is, \textit{problems} whose \textit{solution methods} and/or the initial information required for their solution are a priori unknown)}
...
\begin{frame}{\\Complex problem}
\topline
\justifying
\begin{SCn}
	\scnheader{complex problem}
...
\begin{textitemize}
	\item the problem of understanding natural languages, images, and speech messages
	\item problem of behavior planning for intelligent robots
	\item problem of integrated automation of enterprises
	\item the problem of adaptive learning in various disciplines
\end{textitemize}
\end{frame}
...
\scntext{note}{HIS is focused on solving complex problems}
...
\begin{scnrelfromset}{advantages}
	...
	\scnfileitem{ostis-system can include components developed on the OSTIS base, and be combined with other systems and integrate other components (using JSON or API)}
...
\begin{textitemize}
	...
	\item{the key advantages of OSTIS are not in the new functional capabilities of the systems being developed (most of the functions of ostis-systems can be implemented using traditional tools), but in how easy it is \textbf{to modify and develop} the systems being developed, to adapt them to new problems, and also how effectively the obtained components can be accumulated and used in the development of new systems, while reducing the time and labor intensity of their development}
...
\vspace{10mm}
\begin{figure}[H]
	\includegraphics[width=\textwidth]{./figures/sd_ostis_basics/scheme_en.jpg}
\end{figure}
\end{frame}
...
\justifying
\begin{SCn}
\scnheader{information constuct}
\scnidtf{a structure (structure) containing some information about some entities}	
\scnidtf{information}
...
\begin{scnrelfromset}{subdividing}
	...
	\end{scnindent}
	\scnitem{external information construct that is neither a file nor an sc-structure}
\end{scnrelfromset}
...
\small
\scnheader{discrete information construct}
\scntext{note}{The form of presentation of the elements of a discrete information construct for analyzing its meaning does not require clarification. The main thing is:
\begin{textitemize}
	\item the presence of a simple procedure for isolating (segmenting) elements of a discrete information construct
	\item the presence of a simple procedure for establishing the syntactic equivalence of different elements of a discrete information construct
	\item the presence of a simple procedure for establishing the membership of each element of a discrete information construct in the corresponding class of syntactically equivalent elements (i.e. the corresponding element of the alphabet).
\end{textitemize}
}
\end{SCn}
\end{frame}
...
\begin{SCn}
\scnheader{language}
\scnidtf{a set of information constuctions built according to common syntactic and semantic rules}
\begin{scnrelfromset}{subdividing}
	\scnitem{artificial language}
	\scnitem{natural language}
\end{scnrelfromset}
\scnsuperset{formal language}

\scnheader{formal language}
\scnidtf{a language in which the meaning of each word or sign, the rules for structureing sentences, and understanding their meaning are unambiguous}
\scnidtf{a set of structures composed of some finite alphabet of a language}		
\end{SCn}
\end{frame}
...
\begin{SCn}
	\scnheader{sign}
	\scnidtf{a fragment of an information constuct that has the property of \underline{designating} some entity (object), which, along with other entities, is described by the specified information constuct}
...
By \textbf{\textit{sign}} we mean a fragment of \textit{information construct}, which conventionally represents (denotes) some described entity, which is called \textbf{\textit{denotate of the sign}}.
...
\begin{SCn}
	\scnheader{Semiotics}
	\scnidtf{Science about signs}
	\begin{scnrelfromset}{components}
		\scnitem{Syntax (studies the correct structure of text)}
		\scnitem{Semantics (studies of the meaning of signs)}
...
\justifying
\scnheader{Syntax}
\scnidtf{Science that studies the relationships between signs}
\scntext{note}{The syntax translates to order, coordination}
\scnheader{Semantics}
\scnidtf{Science that studies the relationships between signs and their meanings}
...
\begin{figure}[H]
	\includegraphics[scale=0.5]{./figures/sd_ostis_basics/sc-code-cat_en.jpg}
\end{figure}
\end{SCn}
\end{frame}
